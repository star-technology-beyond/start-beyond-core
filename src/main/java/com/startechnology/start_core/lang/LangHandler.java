...
public class LangHandler {
    ...
    public static void init(RegistrateLangProvider provider) {
        ...
        provider.add("item.start_core.gregtech_multitool.l2", "§7Install tools by combining this with other full durability tools in a crafting table. §7Only §fone§7 tool of a type may be installed at any time.");
        provider.add("item.start_core.gregtech_multitool.l3", "§7Tools can be ejected by holding shift and clicking in the multitool selector. Inserted tools do not keep their enchantments or charge!");
        provider.add("item.start_core.gregtech_multitool.l5", "§aPower Unit:");
        provider.add("item.start_core.gregtech_multitool.l6", "§7When using a tool, the knife consumes energy from its power unit unless the active tool is unbreakable. The power unit must be recharged if it goes down to §b0§7.");
